...
export class InteractionController {
  ...
  private overlayKind: "article" | "search" | null = null;
  ...
  openArticle(number: number, trigger: HTMLElement): void {
    ...
    this.portal.querySelector<HTMLElement>(".close-drawer")?.focus();
  ...
  openSearch(trigger: HTMLElement): void {
    ...
    this.bindCloseControls();
    const input = this.portal.querySelector<HTMLInputElement>("#law-search");
    const results = this.portal.querySelector<HTMLElement>(".search-results");
  ...
  close(restoreFocus = true): void {
    if (!this.overlayKind) return;
    this.portal.innerHTML = "";
    this.overlayKind = null;
    if (restoreFocus) window.requestAnimationFrame(() => this.lastFocused?.focus());
  }

  private bindCloseControls(): void {
    this.portal.querySelectorAll<HTMLElement>("[data-close-overlay]").forEach((element) => {
      element.addEventListener("click", (event) => {
        if (element.hasAttribute("data-close-overlay") && (element.tagName === "BUTTON" || event.target === element)) this.close();
      });
    });
  }
  ...
  private handleOverlayKeydown(event: KeyboardEvent): void {
    ...
    const focusable = Array.from(this.portal.querySelectorAll<HTMLElement>("button, input, [href], [tabindex]:not([tabindex='-1'])"))
