...
function officialParagraph(fullText, fallback) {
    if (!fullText)
        return fallback;
    return fullText
...
async function loadSearchIndex() {
    if (!searchIndexPromise) {
        searchIndexPromise = Promise.all([
            ...
            if (!indexResponse.ok || !fullTextResponse.ok)
                throw new Error("No se pudo cargar el índice de la Ley.");
            return {
                source: await indexResponse.json(),
                fullTexts: await fullTextResponse.json()
            };
        })
...
export async function searchArticles(term) {
    ...
    if (articleMatch)
        return index.filter((article) => article.number === Number(articleMatch[1]));
