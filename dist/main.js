...
if (!appElement)
    throw new Error("No se encontró el contenedor principal de la presentación.");
const app = appElement;
let currentIndex = slideIndexFromHash(window.location.hash);
let loadSequence = 0;
const interaction = new InteractionController();
function boliviaIdentity(compact = false) {
...
function updateChrome() {
    ...
    app.querySelectorAll("[data-slide-index]").forEach((button) => {
    ...
    app.querySelector("[data-action='previous']")?.toggleAttribute("disabled", index === 0);
    app.querySelector("[data-action='next']")?.toggleAttribute("disabled", index === slides.length - 1);
    const progress = app.querySelector(".progress i");
    if (progress)
        progress.style.width = `${((index + 1) / slides.length) * 100}%`;
    app.querySelector(".progress")?.setAttribute("aria-label", `Diapositiva ${index + 1} de ${slides.length}`);
}
async function loadSlide(index, replaceHash = false) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`No se pudo cargar ${selected.file}.`);
        const markup = await response.text();
        if (sequence !== loadSequence)
            return;
        deck.innerHTML = markup;
        deck.querySelector(".slide")?.focus({ preventScroll: true });
...
async function togglePresentation(button) {
    ...
    try {
        ...
        app.querySelector(".app")?.classList.toggle("presenting");
    }
...
app.addEventListener("click", (event) => {
    ...
    const slideButton = target.closest("[data-slide-index]");
    if (slideButton) {
    ...
    if (actionButton) {
        ...
        if (action === "previous")
            changeSlide(-1);
        if (action === "search")
            interaction.openSearch(actionButton);
        if (action === "present")
            void togglePresentation(actionButton);
        return;
    }
    const groupToggle = target.closest("[data-group-toggle]");
    if (groupToggle) {
        const section = groupToggle.closest("[data-group-section]");
...
document.addEventListener("keydown", (event) => {
    if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k") {
        ...
        const trigger = app.querySelector("[data-action='search']");
        if (trigger)
            interaction.openSearch(trigger);
        return;
    }
    if (interaction.isOpen || event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement)
        return;
...
document.addEventListener("fullscreenchange", () => {
    const button = app.querySelector("[data-action='present']");
    if (button)
        button.innerHTML = document.fullscreenElement ? "Salir <span>↙</span>" : "Presentar <span>↗</span>";
});
